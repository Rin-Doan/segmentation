...
def front_crop_or_pad_to_size(volume, target_shape):
    """
    Resize volume to target_shape with axis-aware cropping:
      - Z (axis 0): crop/pad from the front (top-down), preserving the start
      - Y (axis 1): center crop/pad
      - X (axis 2): center crop/pad
    """
    current_shape = np.array(volume.shape)
    target_shape = np.array(target_shape)
    output = np.zeros(target_shape, dtype=volume.dtype)
    ...
    for dim in range(3):
        if dim == 0:  # Z: front crop/pad
            if current_shape[dim] >= target_shape[dim]:
                src_slices.append(slice(0, target_shape[dim]))
                dst_slices.append(slice(None))
            else:
                src_slices.append(slice(None))
                dst_slices.append(slice(0, current_shape[dim]))
        else:  # Y, X: center crop/pad
            if current_shape[dim] >= target_shape[dim]:
                start = (current_shape[dim] - target_shape[dim]) // 2
                src_slices.append(slice(start, start + target_shape[dim]))
                dst_slices.append(slice(None))
...
class Medical3DSegmentationDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Front-crop z / center-crop y,x to target shape
        image_volume = front_crop_or_pad_to_size(image_volume, self.target_shape)
        seg_volume = front_crop_or_pad_to_size(seg_volume, self.target_shape)
        
        # Binary remap: 0 → background, any vertebrae label (1–25) → 1
        seg_volume = np.where(seg_volume > 0, 1, seg_volume)
