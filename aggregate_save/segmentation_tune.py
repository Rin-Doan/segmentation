...
parser.add_argument(
    "--eval",
    action="store_true",
    help="After training finishes, run pixel-wise validation evaluation for this --model (writes evaluation_report_<model>/).",
)
parser.add_argument(
    "--eval-only",
    action="store_true",
    help="Skip training; only run that evaluation using best_<model>_3d_seg.pth in the working directory.",
)
args = parser.parse_args()
if args.eval and args.eval_only:
    parser.error("Choose at most one of --eval and --eval-only.")
...
if args.eval_only:
    ...
    run_evaluation(SELECTED_ARCH, CHECKPOINT_PATH, eval_dir=f"evaluation_report_{SELECTED_ARCH}")
    raise SystemExit(0)
...
num_epochs = 350  # More epochs for 3D (but with fewer samples per epoch)

print("="*60)
print(f"🚀 STARTING 3D SEGMENTATION TRAINING ({SELECTED_ARCH})")
print("="*60)
