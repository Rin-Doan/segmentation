...
MODEL_PATH = 'best_unet_model.pth'  # Use final model (best_unet3d_model.pth if available)
...
def evaluate_model(model, dataloader, num_classes=NUM_CLASSES):
    ...
    with torch.no_grad():
        for images, labels in tqdm(dataloader, desc="Processing 3D volumes"):
            ...
            for pred, target in zip(preds, labels):
                # pred: (D, H, W), target: (D, H, W) - both without channel dimension
    ...
    fps = len(inference_times) / total_inference_time if total_inference_time > 0 else 0  # batches per second
    samples_per_second = len(all_pixel_acc) / total_inference_time if total_inference_time > 0 else 0
    ...
    results = {
        ...
        'inference_stats': {
            ...
            'ms_per_sample': (total_inference_time / len(all_pixel_acc)) * 1000 if len(all_pixel_acc) > 0 else 0
        }
    }
    
    print(f"✓ Evaluation complete on {len(all_pixel_acc)} 3D volumes")
    print(f"⏱️  Inference: {total_inference_time:.2f}s total, {avg_inference_time*1000:.2f}ms/batch avg, {samples_per_second:.2f} volumes/s\n")
    return results
...
def plot_sample_predictions(model, dataset, num_samples=6, save_path='sample_predictions.png'):
    ...
    with torch.no_grad():
        for idx, sample_idx in enumerate(indices):
            ...
            # image: (1, D, H, W), label: (D, H, W), pred: (D, H, W)
            mid_slice = image.shape[1] // 2
            
            img_np = image[0, mid_slice].cpu().numpy()  # Middle slice
            label_np = label[mid_slice].cpu().numpy()  # No squeeze needed now
...
def save_evaluation_report(results, save_path='evaluation_report.txt'):
    ...
    with open(save_path, 'w') as f:
        ...
        if inference_stats:
            ...
            f.write(f"Throughput: {inference_stats.get('samples_per_second', 0):.2f} volumes/second\n")
            f.write(f"Latency per volume: {inference_stats.get('ms_per_sample', 0):.2f} ms\n")
            f.write("\n")
...
def main():
    ...
    # Prepare dataset
    print("Preparing 3D validation dataset...")
    ...
    print(f"  Volume shape: (64, 256, 256)")
    print()
    ...
    print(f"\nGenerated files in '{inference_dir}/' directory:")
    print("  7. inference.csv             - Inference performance metrics")
    print("="*80)
