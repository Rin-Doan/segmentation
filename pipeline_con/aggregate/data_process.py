...
def front_crop_or_pad_to_size(volume, target_shape):
    ...
    Matches generate_bm_data.crop_volume: take slices [0:target_d] and
    zero-pad at the inferior end when the volume is too short. H and W use
    the same center crop/pad as crop_or_pad_to_size.
    """
    current_shape = np.array(volume.shape)
    target_shape = np.array(target_shape)
    output = np.zeros(target_shape, dtype=volume.dtype)
