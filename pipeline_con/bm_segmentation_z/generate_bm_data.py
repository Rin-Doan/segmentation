...
8. Crop the matching 128 slices of the binary mask and save to
   bm_data/bm_masks_nii (used at inference to force the background to class 0).
"""
...
def generate_bm_data():
    ...
    for study_id in tqdm(study_ids, desc="Generating BM data"):
        ...
        try:
            ...
            # Save the cropped binary mask so inference can force background to 0
            # without guessing from intensities (Option 2).
            cropped_mask = crop_volume(binary_mask, first_slice)
            mask_out = os.path.join(OUTPUT_MASK_PATH, f"{study_id}.nii")
            nib.save(
