...
def crop_or_pad_to_size(volume, target_shape):
    """
    Crop or pad volume to target shape with axis-aware alignment.

    - Z (axis 0): top crop / front pad (preserve the start of the volume)
    - Y, X (axes 1, 2): center crop / pad
...
class Medical3DSegmentationDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        seg_volume = np.where(seg_volume > 8, 9, seg_volume)
