...
aggregate_work_50/aggregate_data.py, then binarizes labels (> 1 -> 1).
"""
...
def create_binary_mask(seg_volume):
    """Convert multi-label segmentation to binary mask (pixel > 1 -> 1)."""
    return (seg_volume > 1).astype(np.uint8)
