...
import pydicom
from scipy.ndimage import zoom, rotate, label as ndimage_label
...
OUTPUT_PATH = DATA_PATH + '/triage_database/agg_data'
TARGET_SPACING = (0.5, 0.5, 0.5)  # (z, y, x) in mm
FULL_DATA=False
FIRST_SLICES_CSV = './first_slices.csv'

_CONNECTIVITY_26 = np.ones((3, 3, 3), dtype=np.int32)


def load_first_slice_map(csv_path):
    """Load first_slices.csv and return {study_id: first_slice} dict."""
    mapping = {}
    ...
    with open(csv_path, newline='') as f:
        ...
        for row in reader:
            mapping[row['study_id']] = int(row['first_slice'])
    return mapping


def keep_largest_component(ml_pred: np.ndarray, num_classes: int = None) -> np.ndarray:
    """For each foreground class, retain only the largest 26-connected component."""
    if num_classes is None:
        num_classes = int(ml_pred.max()) + 1
...
def process_study(study_id, target_spacing, first_slice=0, return_data=False):
    """
    ...
    Args:
        ...
        first_slice: Original DICOM slice index to trim from before resampling
    ...
    try:
        ...
        # Trim z-axis to the first vertebra slice BEFORE resampling so the
        # first_slice index (which refers to original DICOM slice numbers) is correct.
        if first_slice > 0:
        ...
        # Trim segmentation to match the image trim (same first_slice index)
        if first_slice > 0:
...
def aggregate_training_data(target_spacing=None, first_slice_map=None, verbose=True):
    """
    ...
    Args:
        ...
        first_slice_map: Dict mapping study_id to first vertebra slice index in the
                         original DICOM volume; trimming happens before resampling
    ...
    for idx, study_id in enumerate(training_studies, 1):
        ...
        image_vol, seg_vol, result = process_study(
            study_id, target_spacing,
            first_slice=first_slice_map.get(study_id, 0),
            return_data=True,
        )
...
def aggregate_data():
    ...
    # Load first-slice map before aggregation so trimming happens before resampling
    first_slice_map = load_first_slice_map(FIRST_SLICES_CSV)
    if first_slice_map:
        print(f"First-slice CSV loaded: {len(first_slice_map)} entries — trimming z-axis before resampling")
    else:
        print("No first-slice CSV found — volumes will not be trimmed on z-axis")
    ...
    for study_id, img_vol, seg_vol in tqdm(zip(study_ids, image_volumes, seg_volumes),
        ...
        if study_id in overlapping_studies:
            seg_vol = keep_largest_component(seg_vol)
