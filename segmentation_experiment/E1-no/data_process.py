...
class MedicalSegmentationDataset(Dataset):
    def __init__(self, study_ids, training_path, segmentation_path, skip_slice, augment=False, augment_p=0.8, transform=None):
    ...
    def _load_and_preprocess_slice(self, dicom_path):
    ...
    def __getitem__(self, idx):
        ...
        image = self._load_and_preprocess_slice(
            sorted_dicom_paths[current_idx])
        
        # Load corresponding segmentation slice (try .nii.gz first, then .nii)
        try:
            ...
            print(f"Error loading segmentation for {sample['study_id']}, slice {sample['seg_slice']}: {e}")
            # Return empty segmentation if loading fails
            seg_slice = np.zeros((512, 512), dtype=np.int64)
        ...
        if image.max() > image.min():
            ...
            # Handle case where image is constant (all same value)
        ...
        if image.shape != (512, 512):
            ...
            seg_slice = zoom(seg_slice, zoom_factors, order=0).astype(np.int64)
        
        # Final validation: ensure labels are in [0, 8] range for 9-class model
        # Clip any negative values to 0 and values > 8 to 8
        seg_slice = np.clip(seg_slice, 0, 8).astype(np.int64)
        # Apply data augmentation (only during training)
        
        if self.augment:
            image, seg_slice = self.augmentor(image, seg_slice)        
        # Ensure arrays are contiguous before converting to tensors
        # This fixes negative stride issues from flips and transposes
        image = np.ascontiguousarray(image)
        seg_slice = np.ascontiguousarray(seg_slice)

        # Convert to tensors
        image_tensor = torch.FloatTensor(image).unsqueeze(0)  # Add channel dimension
        label_tensor = torch.LongTensor(seg_slice)
