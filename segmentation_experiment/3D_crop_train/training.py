...
print(f"Cropped dataset path: {CROPPED_DATASET_DIR}")
...
# Find studies from cropped dataset
# The cropped dataset has structure: cropped_yolo_dataset/{train,val}/images/{study_id}_slice_{i:04d}.png
train_images_dir = os.path.join(CROPPED_DATASET_DIR, 'train', 'images')
val_images_dir = os.path.join(CROPPED_DATASET_DIR, 'val', 'images')

# Extract unique study IDs from cropped dataset filenames
def extract_study_ids_from_dir(images_dir):
    """Extract unique study IDs from cropped dataset filenames"""
    ...
    for filename in os.listdir(images_dir):
        if filename.lower().endswith(('.png', '.jpg', '.jpeg')):
            # Format: {study_id}_slice_{i:04d}.png
            if '_slice_' in filename:
                study_id = filename.rsplit('_slice_', 1)[0]
                study_ids.add(study_id)
    return study_ids

# Get studies from both train and val splits
all_studies = extract_study_ids_from_dir(train_images_dir)
all_studies.update(extract_study_ids_from_dir(val_images_dir))
overlapping_studies = sorted(list(all_studies))
...
# Note: The dataset loader will check both train and val directories when loading,
# so studies can be assigned to either split regardless of where their images are stored
...
print("Note: Dataset loader will search both train and val directories for images")
...
train_dataset = Medical3DSegmentationDataset(
    ...
    split='train',                      # Use 'train' split from cropped dataset
...
val_dataset = Medical3DSegmentationDataset(
    ...
    cropped_dataset_dir=CROPPED_DATASET_DIR,
    split='val',                        # Use 'val' split from cropped dataset
...
num_epochs = 50  # More epochs for 3D (but with fewer samples per epoch)
