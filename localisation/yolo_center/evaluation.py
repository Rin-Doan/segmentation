...
Preprocessing matches Medical3DSegmentationDataset in data_process.py:
NIfTI from agg_data_50, z-trim from first_slices.csv, HU windowing,
front z-crop/pad, and 9-class labels (0=background, 1-7=C1-C7, 8=>7).
Study discovery and train/val split match segmentation.py.
"""
...
from data_process import Medical3DSegmentationDataset, NUM_CLASSES

warnings.filterwarnings("ignore")

DATA_PATH = "/vast/s222440401"
TRAINING_PATH = DATA_PATH + "/agg_data_50/images_nii"
SEGMENTATION_PATH = DATA_PATH + "/agg_data_50/segmentations_nii"
MODEL_PATH = "best_unet3d_model.pth"
TARGET_SHAPE = (128, 256, 256)
RESULTS_CSV = "evaluation_inference_results.csv"
METRICS_PER_CLASS_CSV = "metrics_per_class.csv"
RANDOM_STATE = 42

CLASS_NAMES = ["background", "C1", "C2", "C3", "C4", "C5", "C6", "C7", ">7"]
...
def save_per_class_csv(rows: list[dict], save_path: str = METRICS_PER_CLASS_CSV) -> pd.DataFrame:
    """Write per-class mean Dice/IoU/accuracy table."""
    summary = aggregate_metrics(rows)
    data = {
        "Class": list(range(NUM_CLASSES)),
        "Name": CLASS_NAMES,
        "Dice_Score": [summary[f"dice_{cls}_{CLASS_NAMES[cls]}"] for cls in range(NUM_CLASSES)],
        "IoU_Score": [summary[f"iou_{cls}_{CLASS_NAMES[cls]}"] for cls in range(NUM_CLASSES)],
        "Accuracy": [summary[f"acc_{cls}_{CLASS_NAMES[cls]}"] for cls in range(NUM_CLASSES)],
    }
...
def print_summary(rows: list[dict]) -> None:
    ...
    for cls in range(NUM_CLASSES):
        name = CLASS_NAMES[cls]
        print(f"  {cls} ({name:10s}): {agg[f'dice_{cls}_{name}']:.4f}")
    print("=" * 60)


def main() -> None:
    print("=" * 60)
    print("3D U-Net Evaluation — localisation/yolo")
    print("=" * 60)
    ...
    print(f"Classes: {NUM_CLASSES} (0=background, 1-7=C1-C7, 8=>7)")
    print()
    ...
    save_per_class_csv(rows, METRICS_PER_CLASS_CSV)
    print(f"\nAverage metrics saved to: {RESULTS_CSV}")
    print(f"Per-class table saved to: {METRICS_PER_CLASS_CSV}")
    print_summary(rows)
