...
AUG_NUM = 3
NUM_EPOCHS = 200
NUM_CLASSES = 10   # 0: background, 1-8: C1-T1, 9: other
BATCH_SIZE = 2
MODEL_SHAPE = (256, 256, 256)
...
model = UNet(
    ...
    out_channels=NUM_CLASSES, # Number of segmentation classes (0: background, 1-7: C1-C7, 8: other)
...
train_dataset = Medical3DSegmentationDataset(
    ...
    target_shape=MODEL_SHAPE,        # (D, H, W) output shape
    augment=True,                       # Enable 3D augmentation
    augment_p=1.0,                       # 100% augmentation probability
    n_aug_per_study=AUG_NUM,             # AUG_NUM augmented copies per study
)
...
val_dataset = Medical3DSegmentationDataset(
    ...
    training_path=TRAINING_PATH,   
    segmentation_path=SEGMENTATION_PATH,
    target_shape=MODEL_SHAPE,        # Same shape
...
# Background is already known (input is pre-masked), so we ignore it in the loss and
# focus the optimization purely on distinguishing vertebra levels:
#   - include_background=False -> background channel (0) excluded from the Dice term
#   - weight[0]=0              -> background voxels contribute nothing to the CE term
ce_class_weights = torch.tensor(
    [0.0] + [1.0] * (NUM_CLASSES - 1), device=device
)
criterion = DiceCELoss(
    include_background=False,  # Drop background (channel 0) from the Dice term
    ...
    weight=ce_class_weights,   # Per-class CE weight; 0 disables background
)
...
# Dice metric over foreground classes (1-8), averaged
dice_metric = DiceMetric(include_background=False, reduction="mean")
post_pred = AsDiscrete(argmax=True, to_onehot=NUM_CLASSES)   # one-hot from argmax
post_label = AsDiscrete(to_onehot=NUM_CLASSES)              # one-hot from integer label
...
print(f"Batch size: {batch_size}")
print(f"Input shape: (1, {MODEL_SHAPE[0]}, {MODEL_SHAPE[1]}, {MODEL_SHAPE[2]})")
...
for epoch in range(num_epochs):
    ...
    if val_dice > best_val_dice:
        ...
        torch.save(model.state_dict(), 'best_unet3d_model_ml.pth')
