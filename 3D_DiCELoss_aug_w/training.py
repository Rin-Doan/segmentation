...
image_volumes, seg_volumes, study_ids, report_dict = aggregate_training_data(
    ...
    verbose=True,
    n_aug_per_study=1,   # add 1 offline augmented copy per study to increase samples
)
...
# Class weights to balance C1/C2 (under-represented classes)
# Weights: [Background, C1, C2, C3, C4, C5, C6, C7, Other]
# Higher weights for C1 (10.0) and C2 (8.0) to address class imbalance
class_weights = torch.FloatTensor([
    1.0,   # Class 0: Background
    10.0,  # Class 1: C1 (Atlas) - highest weight due to worst performance
    8.0,   # Class 2: C2 (Axis) - high weight due to poor performance
    1.0,   # Class 3: C3
    1.0,   # Class 4: C4
    1.0,   # Class 5: C5
    1.0,   # Class 6: C6
    1.0,   # Class 7: C7
    1.0    # Class 8: Other vertebrae
]).to(device)

print(f"\nClass weights: {class_weights.cpu().numpy()}")
print(f"  C1 weight: {class_weights[1].item():.1f}x")
print(f"  C2 weight: {class_weights[2].item():.1f}x")
...
# Class weights are applied to both Dice and CrossEntropy components
criterion = DiceCELoss(
    ...
    weight=class_weights  # Class weights to balance C1/C2
)
...
print(f"Loss function: DiceCELoss (Dice + CrossEntropy) with class weights")
print(f"  - C1 weight: {class_weights[1].item():.1f}x")
print(f"  - C2 weight: {class_weights[2].item():.1f}x")
