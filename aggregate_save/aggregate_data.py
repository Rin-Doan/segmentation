...
Return a datset for training and validation
"""
...
def aggregate_training_data(csv_path=None, target_spacing=None, verbose=True):
    """
    ...
    """
    if csv_path is None: csv_path = CSV_PATH
    if target_spacing is None: target_spacing = TARGET_SPACING    
    ...
    if verbose: print(f"Loaded {len(df)} studies from CSV")
    ...
    if verbose: print(f"Found {len(training_studies)} training studies")
    ...
    # Convert results list to dictionary keyed by study_id
...
def main():
    ...
    for study_id, img_vol, seg_vol in tqdm(zip(study_ids, image_volumes, seg_volumes), 
        ...
        if study_id in overlapping_studies:
            # Save segmentation volume as .nii.gz file (NIfTI format)
