...
model = UNet(
    ...
    out_channels=NUM_CLASSES,  # 0: background, 1-7: C1-C7, 8: >7
...
train_dataset = Medical3DSegmentationDataset(
    ...
    target_shape=TARGET_SHAPE,        # (D, H, W) output shape
    ...
    dual_crop=True,                      # front + center crops per study
)
...
val_dataset = Medical3DSegmentationDataset(
    ...
    training_path=TRAINING_PATH,
    segmentation_path=SEGMENTATION_PATH,
    target_shape=TARGET_SHAPE,        # Same shape
    augment=False,                      # No augmentation for validation
    dual_crop=False,                    # one front-crop sample per study (17 val volumes)
)
...
print(f"Output classes: {NUM_CLASSES} (background + C1-C7 + >7)")
print("="*60 + "\n")
