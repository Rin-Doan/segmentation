"""
Pixel-wise 3D U-Net evaluation for the aggregate training pipeline.
...
BATCH_SIZE = 1
NUM_CLASSES = 9
EVAL_DIR = "evaluation_report_aggregate"
...
def save_evaluation_report(results, save_path="evaluation_report.txt"):
    ...
    with open(save_path, "w") as f:
        f.write("=" * 80 + "\n")
        f.write("3D U-NET SEGMENTATION — PIXEL-WISE EVALUATION (aggregate pipeline)\n")
        f.write("=" * 80 + "\n\n")

        f.write("Model: MONAI 3D U-Net\n")
...
def main():
    ...
    print("=" * 80)
    print("3D U-NET — PIXEL-WISE EVALUATION (aggregate / NIfTI pipeline)")
    print("=" * 80)
    print()
    ...
    np.savez(
        os.path.join(EVAL_DIR, "evaluation_metrics.npz"),
        dice_scores=results["dice_scores"],
        iou_scores=results["iou_scores"],
