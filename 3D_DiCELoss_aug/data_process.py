...
class Medical3DSegmentationDataset(Dataset):
    ...
    def _prepare_samples(self):
        ...
        for study_id in self.study_ids:
            ...
            if not os.path.exists(seg_path):
                print(f"Warning: Segmentation not found for {study_id}")
                continue
            
            # Check DICOM directory exists
            study_dir = os.path.join(self.training_path, study_id)
            if not os.path.exists(study_dir):
                print(f"Warning: DICOM directory not found for {study_id}")
                continue
            ...
            if not dicom_files:
                print(f"Warning: No DICOM files found for {study_id}")
                continue
...
# Part 4: Dataset for Pre-Aggregated Data
class Aggregated3DSegmentationDataset(Dataset):
    """
    Dataset for 3D volumetric segmentation using pre-aggregated data
    
    Works with pre-processed volumes from aggregate_data.py
    """
    
    def __init__(self, image_volumes, seg_volumes, study_ids,
                 target_shape=(64, 256, 256),
                 augment=False, augment_p=0.5):
        """
        Args:
            image_volumes: List of numpy arrays (D, H, W) - pre-processed image volumes
            seg_volumes: List of numpy arrays (D, H, W) - pre-processed segmentation volumes
            study_ids: List of study IDs corresponding to volumes
            target_shape: (D, H, W) output volume shape in pixels (for crop/pad)
            augment: Whether to apply data augmentation
            augment_p: Probability of applying augmentation
        """
        assert len(image_volumes) == len(seg_volumes) == len(study_ids), \
            "image_volumes, seg_volumes, and study_ids must have same length"
        ...
        print(f"Created {len(self.image_volumes)} 3D volume samples from pre-aggregated data")
    
    def __len__(self):
        return len(self.image_volumes)
    
    def __getitem__(self, idx):
        # Get pre-processed volumes
        image_volume = self.image_volumes[idx].copy()
        seg_volume = self.seg_volumes[idx].copy()
        
        # Apply HU windowing and normalization to image
        # Note: aggregated data may already be in HU units, so we normalize here
        image_volume = np.clip(image_volume, -200, 1800)
        image_volume = (image_volume - (-200)) / (1800 - (-200))  # Scale to [0, 1]
        
        # Ensure segmentation is int64
        seg_volume = seg_volume.astype(np.int64)
        
        # Crop or pad to target shape if needed
        if image_volume.shape != self.target_shape:
            image_volume = crop_or_pad_to_size(image_volume, self.target_shape)
        if seg_volume.shape != self.target_shape:
            seg_volume = crop_or_pad_to_size(seg_volume, self.target_shape)
